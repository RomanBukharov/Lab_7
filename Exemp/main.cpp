...
void Init_Light()
{
 ...
 GLfloat light_position[] = { 0.0f, 0.0f, -2.0f, 1.0f }; //позици€ источника
...
glLightf(GL_LIGHT0, GL_SPOT_CUTOFF, 50); // конус дл€
//направленного источника
 glLightfv(GL_LIGHT0, GL_SPOT_DIRECTION, light_spot_direction);
 glLightf(GL_LIGHT0, GL_SPOT_EXPONENT, 10); // экспонента
//убывани€ интенсивности
//задействование настроек дл€ источника LIGHT0
...
void drawPrism(float radius, float height, int sides, float xPos, float yPos, float zPos, float transparency) {
    ...
    for (int i = 0; i < sides; ++i) {
        ...
        glColor4f(0.3, 0.4, 0.8, transparency);
        ...
        else
        {
           ...
           glClearColor(0.2f, 0.2f, 0.2f, 0.0f);
            ...
            glPushMatrix();//основные действи€
                ...
                windResize(hwnd);
                ShowWorld();
                drawPrism(1,1,11,0,0,-2,1);
                drawPrism(1,1,11,3,0,-2,0.8);
                drawPrism(1,1,11,6,0,-2,0.6);
                drawPrism(1,1,11,9,0,-2,0.4);
                drawPrism(1,1,11,12,0,-2,0.2);
                drawPrism(1,1,11,15,0,-2,0.1);
            glPopMatrix();
            SwapBuffers(hDC);
            Sleep (1);
        }
